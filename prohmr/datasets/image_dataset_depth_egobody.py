...
class ImageDatasetDepthEgoBody(Dataset):
    ...
    def __init__(self,
        ...
        self.body_pose = self.data['body_pose'].astype(np.float64)[::spacing]  # [n_sample, 69]
        self.betas = self.data['betas'].astype(np.float64)[::spacing]
        self.global_orient_depth = self.data['global_orient_depth'].astype(np.float64)[::spacing]  # [n_sample, 3]
        self.transl_depth = self.data['transl_depth'].astype(np.float64)[::spacing]
        self.keypoints_3d_depth = self.data['3d_joints_depth'].astype(np.float64)[::spacing]
