...
class SMPLFlow(nn.Module):
    ...
    def __init__(self, cfg: CfgNode, contect_feats_dim=None, fc_deep=False):
        """
        ...
        if not fc_deep:
            ...
            self.fc_head = FCHeadDeep(cfg, contect_feats_dim)
    ...
    def forward(self, feats: torch.Tensor, num_samples: Optional[int] = None, z: Optional[torch.Tensor] = None) -> Tuple:
        ...
        pred_pose = rot6d_to_rotmat(pred_pose.reshape(batch_size * num_samples, -1)).view(batch_size, num_samples, self.cfg.SMPL.NUM_BODY_JOINTS+1, 3, 3)  # [bs, 1/num_sample, 24, 3, 3]
