...
class SMPLXFlow(nn.Module):

    def __init__(self, cfg: CfgNode, contect_feats_dim=None):
        """
        Probabilistic SMPL head using Normalizing Flows.
        Args:
            cfg (CfgNode): Model config as yacs CfgNode.
        """
        super(SMPLXFlow, self).__init__()
        self.cfg = cfg
        self.npose = 6*(21 + 1)
        self.flow = ConditionalGlow(132, cfg.MODEL.FLOW.LAYER_HIDDEN_FEATURES,
                                    cfg.MODEL.FLOW.NUM_LAYERS, cfg.MODEL.FLOW.LAYER_DEPTH,
                                    context_features=contect_feats_dim)
        self.fc_head = FCHeadSMPLX(cfg, contect_feats_dim)
    ...
    def log_prob(self, smpl_params: Dict, feats: torch.Tensor) -> Tuple:
        ...
        feats = feats.reshape(batch_size, 1, -1).repeat(1, num_samples, 1)  # [bs, 1, 2048]
    ...
    def forward(self, feats: torch.Tensor, num_samples: Optional[int] = None, z: Optional[torch.Tensor] = None) -> Tuple:
        ...
        pred_pose = pred_params[:, :, :self.npose]  # [bs, 1/num_sample, 132]  self.npose=144
