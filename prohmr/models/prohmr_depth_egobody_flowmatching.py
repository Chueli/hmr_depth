...
from .heads import SMPLXFlowMatching
...
class ProHMRDepthEgobodyFlowmatching(nn.Module):
    ...
    def __init__(self, cfg: CfgNode, device=None, writer=None, logger=None, with_global_3d_loss=False):
        ...
        """
        super(ProHMRDepthEgobodyFlowmatching, self).__init__()
        ...
        self.flow = SMPLXFlowMatching(cfg, context_feats_dim=contect_feats_dim).to(self.device)
    ...
    def compute_loss(self, batch: Dict, output: Dict, train: bool = True) -> torch.Tensor:
        ...
        if smpl_params['body_pose'].shape[0] > 0:
            flow_loss, _ = self.flow.log_prob(smpl_params, conditioning_feats)
            loss_flow_matching = -flow_loss.mean()  # Negative because we minimize
        else:
            loss_flow_matching = torch.zeros(1, device=device, dtype=dtype)
        ...
        loss = self.cfg.LOSS_WEIGHTS['KEYPOINTS_3D_EXP'] * loss_keypoints_3d_exp+ \
               ...
               self.cfg.LOSS_WEIGHTS['NLL'] * loss_flow_matching +\
        ...
        losses = dict(loss=loss.detach(),
                      loss_flow_matching=loss_flow_matching.detach(),
