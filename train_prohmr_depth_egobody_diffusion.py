...
from prohmr.models import ProHMRDepthEgobodyDiffusion
...
def train(writer, logger):
    ...
    # Setup model
    model = ProHMRDepthEgobodyDiffusion(cfg=model_cfg, device=device, writer=None, logger=None, with_global_3d_loss=args.with_global_3d_loss)
    model.train()
    ...
    for epoch in range(args.num_epoch):
        ...
        if epoch == -1 and not hasattr(model, '_phase1_frozen'):
            ...
            model.optimizer = torch.optim.AdamW([
                ...
                {'params': model.diffusion.parameters(), 'lr': model.cfg.TRAIN.LR}
